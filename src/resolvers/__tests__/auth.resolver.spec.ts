...
describe("AuthResolver", () => {
  ...
  let authService: jest.Mocked<Pick<AuthService, "createUser" | "login" | "logout">>;
  ...
  it("logs in and returns tokens", async () => {
    ...
    await expect(
      resolver.login({ emailOrPhone: "mehrdad@example.com", password: "secret" }),
    ).resolves.toEqual(result);
  });

  it("throws UnauthorizedException for failed login", async () => {
    authService.login.mockResolvedValue({ statusCode: 401, message: "Unauthorized" });
  ...
  ])("maps logout status %s to GraphQL exception", async (statusCode, exception) => {
    authService.logout.mockResolvedValue({ statusCode, message: "Error" });

    await expect(resolver.logout("refresh-token")).rejects.toBeInstanceOf(exception);
  });
});
