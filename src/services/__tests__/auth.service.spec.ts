...
describe("AuthService", () => {
  ...
  it("sends create user payload to auth.createUser", async () => {
    ...
    expect(rmq.requestJson).toHaveBeenCalledWith("auth.createUser", input, 10_000);
  });

  it("maps login input to backend username payload", async () => {
    rmq.requestJson.mockResolvedValue({ accessToken: "access", refreshToken: "refresh" });

    await service.login({ emailOrPhone: "mehrdad@example.com", password: "secret" });
  ...
  it("sends logout refresh token to auth.logout", async () => {
    rmq.requestJson.mockResolvedValue({ statusCode: 200, message: "Logged out" });
