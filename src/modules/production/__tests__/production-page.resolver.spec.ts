...
describe('ProductionPageResolver', () => {
  ...
  let productionService: jest.Mocked<
    Pick<
      ProductionService,
      'getGridSummery' | 'getProductionChart' | 'getFildCompare'
    >
  >;
  ...
  beforeEach(() => {
    ...
    resolver = new ProductionPageResolver(
      productionService as ProductionService,
    );
  });
  ...
  it('returns production chart for requested history', async () => {
    const result = {
      labels: ['Jan', 'Feb'],
      datasets: [
        { label: 'Oil', data: [100, 120], backgroundColor: '#0ea5e9' },
      ],
    };
    productionService.getProductionChart.mockResolvedValue(result);

    await expect(resolver.getProductionChart('monthly')).resolves.toEqual(
      result,
    );
  });
